...
describe Slither::Section do
  let(:section) { Slither::Section.new(:body) }
  ...
  it "should accept and store the trap as a block" do
    section.trap { |v| v == 4 }
    t = section.instance_variable_get(:@trap)
    t.should be_a(Proc)
    t.call(4).should == true
  end

  describe "when adding a template" do
    let(:template) { double('templated section', :columns => [1,2,3], :options => {}) }
    let(:definition) { double("definition", :templates => { :test => template } ) }
  ...
  describe "when parsing a file" do
    ...
    let(:section) { Slither::Section.new(:body) }
    let(:column_content) { { :id => 5, :first => 10, :last => 10, :state => 2 } }
